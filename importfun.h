...
{
public:
	thread_operation(DWORD dwDesiredAccess, DWORD threadId, BOOL bWowProcess);
	~thread_operation();
	HANDLE OpenThread();
	BOOL SuspendThread();
	BOOL ResumeThread();
	BOOL WINAPI SetThreadContext(CONTEXT* lpContext);
	BOOL WINAPI GetThreadContext(CONTEXT* lpContext);
	PVOID GetThreadTebBaseAddress64();

private:

	DWORD m_threadId = 0;
	HANDLE m_threadHanle = 0;
	BOOL m_bWowProcess = FALSE;
...
BOOL CreateRemoteThread_(HANDLE hProcess, PVOID pExcuteAddress, PVOID pArg = NULL, PHANDLE pThreadHanle = NULL);
NTSTATUS  ZwQuerySystemInformation_(
...
NTSTATUS ZwQueryInformationProcess_(
	IN HANDLE ProcessHandle,
	IN PROCESSINFOCLASS ProcessInformationClass,
	OUT PVOID ProcessInformation,
	IN ULONG ProcessInformationLength,
...
BOOL WINAPI WriteProcessMemory_(
	HANDLE hProcess,
	LPVOID lpBaseAddress,
	PVOID lpBuffer,
	SIZE_T nSize,
	SIZE_T* lpNumberOfBytesWritten);
BOOL WINAPI ReadProcessMemory_(HANDLE hProcess, LPVOID lpBaseAddress, LPVOID lpBuffer, SIZE_T nSize, SIZE_T* lpNumberOfBytesRead);
...
NTSTATUS  ZwQueryInformationThread_ (
