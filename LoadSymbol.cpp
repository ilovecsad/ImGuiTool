...
void find_sym_rva_ex(const PDB::RawFile& rawPdbFile, const PDB::DBIStream& dbiStream, vector<symbol_info>& vectorsymbolInfo,string moduleName)
{

	vectorsymbolInfo.clear();
	...
	for (const PDB::HashRecord& hashRecord : hashRecords)
	{
		...
		if (record->data.S_PUB32.name) 
		{
			dwInfo.Belonging_to_module = moduleName;
			dwInfo.rva = rva;
			dwInfo.szFuncName = record->data.S_PUB32.name;
...
bool Symbol::init(void* pdbFile_baseAddress,vector<symbol_info>& vectorsymbolInfo,string moduleName) 
{
	// std::hash<std::string> strhash;
	...
	find_sym_rva_ex(rawPdbFile, dbiStream, vectorsymbolInfo, moduleName);
...
BOOL Symbol::LoadSymbol(string szSaveFileName,vector<symbol_info>& vectorsymbolInfo)
{
	std::vector<uint8_t> data;

	BOOL bRet = FALSE;

	string symbol_name = szSaveFileName;

	symbol_name = symbol_name + ".pdb";



	if (!check_symbol_file(symbol_name)) 
	{
		string pdb_url = pdburl(GetModuleDebugInfo(m_szFullPath.c_str()));

		if (MessageBoxW(NULL, L"需要连接微软服务器 下载pdb-这会花费大量的时间", L"提示", MB_OKCANCEL) == IDOK) {

			auto hr = URLDownloadToFileA(0, pdb_url.c_str(), symbol_name.c_str(), 0, NULL);
			if (hr != S_OK)
			{
				MessageBoxW(NULL, L"下载符号失败!", L"提示", MB_OKCANCEL);
				return bRet;
			}
		}
		else 
	...
	open_binary_file(symbol_name, data);

	bRet =init(data.data(),vectorsymbolInfo,m_szDllBasePath);
